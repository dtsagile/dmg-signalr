...
/* Chirpy Minify: True, MinifyWith: Unspecified, File: D:\SVN\damageassessment\DamageAssessment\DamageAssessment\Public\javascript\microevent.js */
/**
...
/* Chirpy Minify: True, MinifyWith: Unspecified, File: D:\SVN\damageassessment\DamageAssessment\DamageAssessment\Public\javascript\iowrapper.js */
//This depends on socket.io being loaded into the page
IOWrapper = function() {
    console.log('initing IOWrapper...');
    var self = this;
    this._session = Map.Session;

    this._realTimeHub = $.connection.realTimeHub;
    //listen for events coming from the server    
    this._realTimeHub.gotMapExtent = function(data) {
        console.log('IOWrapper: got updateMap...');
        self.trigger('updateMap', data);
    };

    this._realTimeHub.gotNewDamageLocation = function(data) {
    ...
    $.connection.hub.start(function() {
        ...
        /*self._session && self._session.exists() && */
        self._realTimeHub.getMapPoints().done(function(data) {
            $.each(data, function(idx, item) {
                self.trigger('pointAdded', item);
            });
        });
    });
    ...
    this.addMapPoint = function(data) {
        this._realTimeHub.addDamageLocation(data.lat, data.lng, data.pointType);
    };

    this.pushMapState = function(data) {
        this._realTimeHub.updateMapExtent(data.lat, data.lng, data.zoom);
    };

    this.getPoints = function() {
        this._realTimeHub.getMapPoints().done(function(data) {
...
/* Chirpy Minify: True, MinifyWith: Unspecified, File: D:\SVN\damageassessment\DamageAssessment\DamageAssessment\Public\javascript\map.controller.js */
// Namespace
...
Map.Controller = (function($) {
    ...
    //PRIVATE VARIABLES
    var me = {},
        _map, _disasterLayer, _ioWrapper = {},
        _addPointMapHandler = {},
        _isDriver = false,
        _currentMarkerType = '',
        _imageBaseUrl, _session = Map.Session;
    ...
    function _getMarker(markerType) {
        var iconUrl = _imageBaseUrl + markerType + '.png',
            icon = L.Icon.extend({
    ...
    me.init = function(args) {
        ...
        _initMap();
        _initIO();
        $('.controls').on('click', '#toggle-drive', function() {
            _isDriver = !_isDriver;
            var text = (_isDriver) ? 'Stop Driving' : 'Drive Map';
            $(this).toggleClass('ui-btn-active').text(text);
        });
